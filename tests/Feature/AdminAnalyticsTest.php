...
test('analytics includes every day every metric and readable arabic dates', function () {
    ...
    expect($rows)->toHaveCount(7)
        ->and($rows->first()['date']->toDateString())->toBe('2026-08-09')
        ->and($rows->last()['date']->toDateString())->toBe('2026-08-15')
        ->and($rows->get(4)['site_visits'])->toBe(9)
        ->and($rows->get(5)['site_visits'])->toBe(0)
