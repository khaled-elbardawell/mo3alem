...
test('the certificate tool presents templates upload editing preview and print controls', function () {
    $response = $this->get(route('tools.certificates'))
        ...
        ->assertSee('id="saveCertificateButtonLabel"', false)
        ->assertSee('id="renameCertificateTitleBtn"', false)
...
test('the certificate client supports draft handoff manipulation and printable export', function () {
    ...
    expect($script)
        ...
        ->toContain('syncCertificateTitleUi')
        ->toContain('renameButton.addEventListener("click", openSaveFlow)')
        ->toContain('saveButtonLabel.textContent = isSavedCertificate ? "تعديل" : "حفظ"')
