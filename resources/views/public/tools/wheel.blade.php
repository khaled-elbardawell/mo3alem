    ...
    <div
        ...
        <div id="wheel">
            ...
            <div class="fixed inset-0 z-[120] hidden items-center justify-center bg-slate-950/45 p-5 backdrop-blur-sm"
                ...
                <div class="grid w-[min(100%,390px)] justify-items-center gap-4 rounded-3xl border border-white/60 bg-white p-6 text-center shadow-2xl"
                    ...
                    <div class="grid gap-1">
                        <strong class="text-lg font-black text-slate-900" id="importLoaderTitle">جارٍ قراءة
                            الملف…</strong>
                    ...
                    <div class="h-2.5 w-full overflow-hidden rounded-full bg-slate-100" role="progressbar"
                        ...
                        <span
                            class="block h-full w-0 rounded-full bg-linear-to-l from-violet-700 to-indigo-500 transition-[width] duration-200"
            ...
            <section @class([
                ...
                'w-[min(calc(100%_-_44px),1320px)] grid-cols-[250px_minmax(360px,1fr)_410px]' =>
                    $campaigns['side'],
                'w-[min(calc(100%_-_44px),1040px)] grid-cols-[minmax(360px,1fr)_410px]' => !$campaigns[
                    'side'
                ],
                ...
                <section
                    ...
                    <div class="wheel-wrap relative aspect-square w-[var(--wheel-size)] [--wheel-size:min(100%,520px)] before:absolute before:inset-[9%] before:-z-10 before:animate-wheel-halo before:rounded-full before:bg-[radial-gradient(circle,rgba(51,65,85,0.16),transparent_62%)] before:opacity-[0.55] before:blur-[10px] before:content-[''] [&.is-few]:[--wheel-size:min(100%,570px)] [&.is-many]:[--wheel-size:min(100%,485px)] max-[900px]:[--wheel-size:min(100%,440px)] max-[900px]:[&.is-few]:[--wheel-size:min(100%,480px)] max-[900px]:[&.is-many]:[--wheel-size:min(100%,400px)] max-[620px]:[--wheel-size:min(100%,342px)] max-[620px]:[&.is-few]:[--wheel-size:min(100%,342px)] max-[620px]:[&.is-many]:[--wheel-size:min(100%,342px)] [.wheel-stage:fullscreen_&]:[--wheel-size:min(74vmin,720px)]"
                        ...
                        <canvas
                            ...
                            id="wheelCanvas" width="720" height="720" aria-label="عجلة اختيار الأسماء"></canvas>
                        <button
                            ...
                            type="button" id="centerSpinBtn" aria-label="حرك العجلة">
                    ...
                    <div
                        ...
                        <button
                            ...
                            type="button" id="spinBtn" aria-label="حرك العجلة واختيار اسم">
                            <span class="spin-btn__text">حرك العجلة</span>
                            <i class="fa-solid fa-rotate"></i>
                        </button>
                    </div>
                    ...
                    <div
                        ...
                        <label class="inline-flex items-center gap-2">
                            ...
                            <span>تحريك تلقائي بعد</span>
                        </label>
                        <label class="inline-flex items-center gap-1">
                            <span class="sr-only">مدة التحريك التلقائي بالثواني</span>
                            <input
                                class="h-9 w-16 rounded-lg border border-slate-200 bg-white px-2 text-center font-black text-slate-700 outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-100 disabled:bg-slate-100 disabled:opacity-60"
                                id="autoSpinDelay" type="number" min="1" max="60" step="1" value="5"
                                inputmode="numeric" aria-label="مدة التحريك التلقائي بالثواني" />
                            <span>ثواني</span>
                        </label>
                    </div>
                ...
                <aside
                        ...
                        <section class="mb-3.5 hidden rounded-2xl border border-violet-100 bg-white p-3.5 shadow-sm"
                            ...
                            <div class="mt-3 grid gap-3" id="guestSaveActions">
                                ...
                                <p class="m-0 text-center text-xs font-bold text-slate-500">
                                    ليس لديك حساب؟
                                    <a class="text-violet-700 underline" href="{{ route('register') }}">أنشئ حسابًا
                                        مجانًا</a>
                                </p>
                            </div>
                        </section>
                    ...
                    @auth
                        ...
                        <section @class([
                            'grid gap-3.5',
                            'hidden' =>
                                filled($wheelConfig['savedWheel']) ||
                                filled($wheelConfig['competition']),
                        ]) id="competitionsBrowser"
                            aria-labelledby="competitionsBrowserTitle">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <h3 class="m-0 text-lg font-black text-slate-900" id="competitionsBrowserTitle">مسابقاتي
                                    </h3>
                                    <p class="mt-1 text-xs font-bold leading-5 text-slate-500">افتح مسابقة سابقة أو ابدأ مسابقة
                                        جديدة بخطوتين.</p>
                                </div>
                                <button
                                    ...
                                    id="createCompetitionBtn" type="button" @disabled(!auth()->user()->hasVerifiedEmail())>
                            ...
                            <p class="m-0 min-h-5 text-xs font-bold text-slate-500" id="competitionsStatus" role="status"
                                aria-live="polite"></p>
                        ...
                        <section @class([
                        ...
                        ]) id="savedWheelsBrowser"
                            aria-labelledby="savedWheelsBrowserTitle">
                            <div class="flex items-center justify-between gap-3">
                                <div>
                                    <h3 class="m-0 text-lg font-black text-slate-900" id="savedWheelsBrowserTitle">قوائمي
                                        المحفوظة</h3>
                                    <p class="mt-1 text-xs font-bold text-slate-500">ابحث عن قائمة أو أنشئ واحدة جديدة للبدء.
                                    </p>
                                </div>
                                <button
                                    class="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-violet-700 px-3.5 text-sm font-black text-white shadow-sm transition hover:bg-violet-800 disabled:cursor-not-allowed disabled:opacity-55"
                                    id="createSavedWheelBtn" type="button" @disabled(!auth()->user()->hasVerifiedEmail())>
                                    <i class="fa-solid fa-plus" aria-hidden="true"></i>
                                    قائمة جديدة
                                </button>
                            </div>
                            ...
                            <p class="m-0 min-h-5 text-xs font-bold text-slate-500" id="savedWheelsStatus" role="status"
                                aria-live="polite"></p>
                    ...
                    <div @class([
                        ...
                        'hidden' =>
                            auth()->check() &&
                            blank($wheelConfig['savedWheel']) &&
                            blank($wheelConfig['competition']),
                    ]) id="wheelEditor">
                        @auth
                            <div class="mb-3.5 grid gap-2.5" id="savedWheelEditorHeader">
                                ...
                                <div class="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-x-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2.5"
                                    ...
                                    <span
                                        class="row-span-2 grid h-8 w-8 place-items-center rounded-lg bg-white text-emerald-700"
                                    ...
                                    <span class="text-xs font-bold text-emerald-700" id="activeWorkspaceHint">يتم حفظ
                                        التغييرات تلقائيًا</span>
                                </div>

                                <p class="m-0 min-h-5 text-xs font-bold text-slate-500" id="saveStatus" role="status"
                                    aria-live="polite"></p>
                                ...
                                <div class="hidden rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"
                                    ...
                                    <div class="mt-2 flex gap-3">
                                        <button class="font-black underline" id="reloadConflictBtn" type="button">تحميل نسخة
                                            الخادم</button>
                                        <button class="font-black underline" id="copyConflictBtn" type="button">حفظ الحالية
                                            كنسخة</button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="panel-tabs mb-3.5 grid grid-cols-2 gap-2.5 [&_button]:inline-flex [&_button]:min-h-[46px] [&_button]:cursor-pointer [&_button]:items-center [&_button]:justify-center [&_button]:gap-2 [&_button]:rounded-[14px] [&_button]:border [&_button]:border-[#e7e2f0] [&_button]:bg-[#fafafa] [&_button]:font-black [&_button]:text-[#344054] [&_button.active]:border-[#d8c7ff] [&_button.active]:bg-white [&_button.active]:text-[#6d28d9] [&_button.active]:shadow-[inset_0_-3px_0_#6d28d9] max-[620px]:[&_button]:min-h-11 max-[620px]:[&_button]:text-sm"
                            ...
                            <button type="button" data-tab="results" id="resultsTab" role="tab"
                                aria-selected="false" aria-controls="resultsPage" tabindex="-1">
                        ...
                        <div class="tab-page active hidden [&.active]:block" id="dataPage" role="tabpanel"
                            ...
                            <div class="mb-3 grid grid-cols-2 gap-2.5 rounded-2xl border border-violet-100 bg-violet-50 p-2.5 sm:grid-cols-3"
                                ...
                                <button
                                    class="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-violet-200 bg-white px-3 py-2.5 text-sm font-black text-violet-700 shadow-sm transition hover:border-violet-300 hover:bg-violet-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-600 disabled:cursor-not-allowed disabled:opacity-55"
                                    id="importTrigger" type="button" aria-controls="importLoader"
                                    aria-disabled="false">
                                    <i class="fa-solid fa-file-arrow-up" aria-hidden="true"></i>
                                    استيراد أسماء
                                </button>
                                <input class="sr-only" id="importInput" type="file"
                            ...
                            <div class="virtual-list relative mt-3 h-[282px] overflow-auto rounded-2xl border border-[#e7e2f0] bg-white [&.is-empty]:grid [&.is-empty]:place-items-center [&::-webkit-scrollbar]:w-[9px] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#c4bdce] max-[900px]:h-[260px] [&_.name-row]:flex [&_.name-row]:h-[46px] [&_.name-row]:cursor-grab [&_.name-row]:items-center [&_.name-row]:gap-2 [&_.name-row]:border-b [&_.name-row]:border-[#f0eef7] [&_.name-row]:pr-2.5 [&_.name-row]:pl-3 [&_.name-row]:[direction:rtl] [&_.name-row]:transition-[background,box-shadow] [&_.name-row]:duration-200 [&_.name-row.is-dragging]:cursor-grabbing [&_.name-row.is-dragging]:opacity-[0.55] [&_.name-row.is-drop-target]:bg-[#f7f2ff] [&_.name-row.is-drop-target]:shadow-[inset_0_0_0_2px_rgba(51,65,85,0.16)] [&_.name-row_input]:accent-[#6d28d9] [&_.name-row__order]:min-w-7 [&_.name-row__order]:shrink-0 [&_.name-row__order]:text-center [&_.name-row__order]:text-xs [&_.name-row__order]:font-black [&_.name-row__order]:text-[#667085] [&_.name-row__name]:flex-1 [&_.name-row__name]:overflow-hidden [&_.name-row__name]:text-ellipsis [&_.name-row__name]:whitespace-nowrap [&_.name-row__name]:font-bold [&_.name-row__name]:text-[#344054] [&_.name-row__actions]:inline-flex [&_.name-row__actions]:shrink-0 [&_.name-row__actions]:items-center [&_.name-row__actions]:gap-[5px] [&_.name-row__btn]:grid [&_.name-row__btn]:h-[30px] [&_.name-row__btn]:w-[30px] [&_.name-row__btn]:cursor-pointer [&_.name-row__btn]:place-items-center [&_.name-row__btn]:rounded-[9px] [&_.name-row__btn]:border [&_.name-row__btn]:border-[#e7e2f0] [&_.name-row__btn]:bg-white [&_.name-row__btn]:text-[13px] [&_.name-row__btn]:text-[#1e293b] [&_.name-row__btn]:transition-[background,border-color,color,transform] [&_.name-row__btn]:duration-200 [&_.name-row__btn:hover:not(:disabled)]:-translate-y-px [&_.name-row__btn:hover:not(:disabled)]:border-[#cbd5e1] [&_.name-row__btn:hover:not(:disabled)]:bg-[#f4efff] [&_.name-row__btn:disabled]:cursor-not-allowed [&_.name-row__btn:disabled]:opacity-[0.38] [&_.name-row__btn--danger]:text-[#ef4444] [&_.name-row__btn--danger:hover:not(:disabled)]:border-[rgba(239,68,68,0.25)] [&_.name-row__btn--danger:hover:not(:disabled)]:bg-[#fff1f2] max-[620px]:[&_.name-row]:gap-1.5 max-[620px]:[&_.name-row]:px-2 max-[620px]:[&_.name-row__actions]:gap-[3px] max-[620px]:[&_.name-row__btn]:h-7 max-[620px]:[&_.name-row__btn]:w-7 max-[620px]:[&_.name-row__btn]:rounded-lg max-[620px]:[&_.name-row__btn]:text-xs"
                                ...
                                <div class="empty-state empty-state--names grid w-[min(100%,360px)] justify-items-center gap-1.5 rounded-2xl border border-dashed border-[rgba(51,65,85,0.2)] bg-[#fbf9ff] px-4 py-[18px] text-center text-[#667085] [&[hidden]]:hidden"
                                    ...
                                    <p class="m-0 leading-[1.7] font-bold">استخدم زري الإضافة أو الاستيراد أعلى القائمة
                                        للبدء.
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="tab-page hidden [&.active]:block" id="resultsPage" role="tabpanel"
                            ...
                            <div class="empty-state empty-results mx-auto mt-3 grid w-[min(100%,360px)] justify-items-center gap-1.5 rounded-2xl border border-dashed border-[rgba(51,65,85,0.2)] bg-[#fbf9ff] px-4 py-[18px] text-center text-[#667085] [&[hidden]]:hidden"
                                ...
                                <p class="m-0 leading-[1.7] font-bold">حرك العجلة لإضافة أول فائز إلى القائمة.</p>
                            </div>
                        </div>
                    </div>
                </aside>
            ...
            <section
                ...
                <div class="steps-grid mt-7 grid grid-cols-4 gap-[18px] max-[1180px]:grid-cols-2 max-[620px]:grid-cols-1">
                    ...
                    <article
                        ...
                        <h3 class="mt-4 mb-2 text-[19px] font-black">حرك العجلة</h3>
            ...
            <section
                ...
                <div class="save-mode__content min-w-0">
                    <h2
                        class="mt-3 mb-0 text-[clamp(24px,3vw,34px)] leading-[1.35] font-black text-[#1e293b] max-[620px]:text-2xl">
                    ...
                    <div
                        ...
                        @auth
                            <a class="btn btn--primary inline-flex min-h-11 items-center justify-center gap-[9px] rounded-xl bg-violet-700 px-5 py-2 font-extrabold text-white!"
                                href="{{ route('dashboard') }}">إدارة مسابقاتي وقوائمي <i
                                    class="fa-solid fa-bookmark"></i></a>
                        @else
                            <a class="btn btn--primary inline-flex min-h-11 items-center justify-center gap-[9px] rounded-xl bg-violet-700 px-5 py-2 font-extrabold text-white!"
                                href="{{ route('login') }}">سجل دخول لتفعيل وضع الحفظ <i class="fa-solid fa-user"></i></a>
                        @endauth
                    </div>
                </div>
            </section>
        ...
        @auth
            <dialog
                ...
                <form class="w-[min(560px,calc(100vw_-_32px))] p-5 sm:p-6" id="createCompetitionForm">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <h3 class="m-0 text-xl font-black text-slate-900" id="createCompetitionDialogTitle">مسابقة جديدة
                            </h3>
                        ...
                        <button
                            class="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-900"
                    ...
                    @if (auth()->user()->hasVerifiedEmail())
                        <div class="mt-5 grid gap-4">
                            <label class="grid gap-1.5 text-sm font-black text-slate-700">
                                اسم المسابقة
                                <input
                                    class="min-h-12 rounded-xl border border-slate-200 px-3.5 outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-100"
                            ...
                            <fieldset class="grid gap-3">
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <label
                                        class="flex min-h-12 items-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-3 text-sm font-black text-violet-800">
                                        <input class="accent-violet-700" type="radio" name="competitionListMode"
                                            value="existing" checked>
                                        اختيار قائمة محفوظة
                                    </label>
                                    <label
                                        class="flex min-h-12 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-sm font-black text-slate-700">
                                        <input class="accent-violet-700" type="radio" name="competitionListMode"
                                            value="new">
                                        إنشاء قائمة جديدة
                                    </label>
                            ...
                            <div class="grid gap-2.5" id="competitionExistingListPanel">
                                <label class="relative block">
                                    ...
                                    <i class="fa-solid fa-magnifying-glass pointer-events-none absolute top-1/2 right-3.5 -translate-y-1/2 text-slate-400"
                                        aria-hidden="true"></i>
                                    <input
                                        class="min-h-11 w-full rounded-xl border border-slate-200 pr-10 pl-3 text-sm font-bold outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-100"
                                ...
                                <button
                                    class="hidden min-h-10 rounded-xl border border-slate-200 bg-white text-sm font-black text-violet-700 hover:bg-violet-50"
                                    id="loadMoreCompetitionListsBtn" type="button">تحميل المزيد</button>
                                <p class="m-0 text-xs font-bold text-slate-500" id="competitionListsStatus" role="status"
                                    aria-live="polite"></p>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-2.5">
                                <button
                                    class="min-h-11 rounded-xl border border-slate-200 bg-white px-4 font-black text-slate-600 hover:bg-slate-50"
                                    type="button" data-close-competition-dialog>إلغاء</button>
                                <button
                                    class="min-h-11 rounded-xl bg-violet-700 px-4 font-black text-white hover:bg-violet-800 disabled:cursor-wait disabled:opacity-60"
            ...
            <dialog
                ...
                <form class="w-[min(440px,calc(100vw_-_32px))] p-5 sm:p-6" id="createSavedWheelForm">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <h3 class="m-0 text-xl font-black text-slate-900" id="createSavedWheelDialogTitle">إنشاء قائمة
                                جديدة</h3>
                            <p class="mt-1 text-sm font-bold leading-6 text-slate-500">
                    ...
                    @if (auth()->user()->hasVerifiedEmail())
                        <div class="mt-5 grid gap-4">
                            <div
                                class="flex items-center gap-3 rounded-xl bg-violet-50 px-3 py-2.5 text-sm font-bold text-violet-800">
