...
@section('content')
    ...
    <p class="mt-3 leading-7 text-slate-600">أرسلنا رابط التفعيل إلى بريدك. ستبقى مسودتك المحلية محفوظة، وبعد التفعيل سنعيدك إلى الأداة التي كنت تستخدمها.</p>
    ...
    <div class="mt-4 grid grid-cols-2 gap-3">
        <a class="rounded-xl border border-violet-200 px-4 py-3 text-center font-bold text-violet-700" href="{{ str_contains(session('url.intended', ''), '/tools/qr') ? route('tools.qr') : route('tools.wheel') }}">
            {{ str_contains(session('url.intended', ''), '/tools/qr') ? 'العودة إلى تصميم QR' : 'متابعة للعجلة' }}
        </a>
